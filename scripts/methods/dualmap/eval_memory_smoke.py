...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Read a DualMap memory package and run a smoke eval.")
...
def main(args: argparse.Namespace) -> int:
    ...
    output = args.output
    if output is None:
        output = timestamped_result_dir("dualmap", "dualmap-memory-smoke") / "eval_summary.json"
...
def _latest_package(scene_id: str) -> Path:
    root = REPO_ROOT / "memories" / "dualmap" / "scannetpp" / scene_id
    candidates = sorted([path for path in root.glob("*") if path.is_dir()])
    if not candidates:
        raise FileNotFoundError(f"no DualMap packages found under {root}")
    return candidates[-1]
