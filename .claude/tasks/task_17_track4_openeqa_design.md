# Task 17: Track 4 OpenEQA Design

## Goal

Draft the implementation design for OpenEQA adaptation after Track 1/2 are
stable.

## Scope

Design only unless explicitly asked to implement. The final evaluator and LLM
judge setup are human-owned.
...
- GT answers go only to scorer/judge, never to fixed API entrypoint or agent
  sandbox.
- Fixed API support requires a native QA/retrieval API.
- Do not add a generic object-table-to-LLM answerer and count it as method
  support.
- LLM judge must be separate from method memory construction modules.

## Deliverables

- Design doc or section covering OpenEQA package mapping, fixed API eligibility,
  agentic prompt shape, scoring, evidence audit, and leakage policy.
