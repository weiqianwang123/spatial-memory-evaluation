...
~0.044 秒是它自己的每帧 CV+tracking 成本(取自 `performance_statistics.csv`,10 场景均值);它笨重的
DAM-3B grounding 描述器是**异步运行、不阻塞每帧路径**的,所以流式循环保持很快(这是 DAAAM 的实时 SLAM
设计),但描述器本身的延迟没算进这个数。ClawS 的 ~0.10 秒是它从预建 DB 得到的 `process_frame` 成本。
ReMEmbR 只描述约 6% 的帧(~1 帧/3 秒)、每次约 5 秒 → 归一化到视频时间轴约每视频帧 0.29 秒。所以 AI
自设计的记忆是**接近实时但不是最快** —— DAAAM 的纯几何循环更快;之前那个 "0.012 秒" 的 DAAAM 数值
(来自只打包的 `--skip-daaam-run` 日志)是错的。请勿把这一列解读为"某方法比另一个实时 N 倍"。
